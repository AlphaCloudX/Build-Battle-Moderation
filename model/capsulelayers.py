    ...
    def call(self, inputs, training=None):
        ...
        for i in range(self.routings):
            ...
            if i < self.routings - 1:
                ...
                b = tf.add(b, agreement) # <--- these are the raw weights we need to return for rpv
...
def PrimaryCap(inputs, dim_capsule, n_channels, kernel_size, strides, padding, activation):
    """
    ...
    output = layers.Conv2D(filters=dim_capsule*n_channels, kernel_size=kernel_size, strides=strides, padding=padding,
                           name='primarycap_conv2d', activation=activation)(inputs)

    outputs = layers.Reshape(target_shape=[-1, dim_capsule], name='primarycap_reshape')(output)

    return layers.Lambda(squash, name='primarycap_squash')(outputs), output
